...
async function findRelevantVerses(query) {
    try {
        if (!bibleCache) {
            const bibleRes = await fetch("https://raw.githubusercontent.com/yunhoshin87/bib/main/bible.md");
            if (bibleRes.ok) {
                const bibleText = await bibleRes.text();
                bibleCache = bibleText.split('\n').filter(line => line.match(/^\d+\./));
            }
        }
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/chat" && request.method === "POST") {
      try {
        ...
        if (!api_key) return new Response(JSON.stringify({ response: "에러: API 키가 설정되지 않았습니다." }), { 
            status: 200, 
        ...
        const response = await fetch(geminiUrl, {
          ...
          body: JSON.stringify({
            ...
            system_instruction: { parts: [{ text: "너는 부드럽고 인자한 영적 동반자 '진리'야. 성도들의 고민을 들으면 차분하고 정중하게 '성도님...' 하며 대화를 시작해줘. 반드시 성경 속의 구체적인 예시를 들어 위로해주고 성경 구절을 인용해줘. 답변 마지막에는 항상 따뜻한 축복의 한마디를 덧붙여줘." }] }
          })
        });
        ...
        if (!data.candidates || data.candidates.length === 0) {
            return new Response(JSON.stringify({ response: "죄송합니다. 적절한 말씀을 찾지 못했습니다. 다시 말씀해 주시겠어요?" }), {
                headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
            });
        }
        ...
        async function sendMessage() {
            ...
            try {
                ...
                if (data.response) { 
                    ...
                    appendMessage('ai', 'API 오류: ' + data.error);
                }
            } catch (e) { 
                console.error(e);
                appendMessage('ai', '죄송합니다. 연결에 문제가 발생했습니다: ' + e.message); 
            }
        }
