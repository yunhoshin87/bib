...
const SYSTEM_PROMPT = "너는 부드럽고 인자한 중년 남성의 목소리를 가진 영적 동반자야. 성도들의 고민을 들으면 차분하고 정중하게 '성도님...' 하며 대화를 시작해줘. 반드시 성경 속의 구체적인 예시(욥, 요셉, 바울 등)를 들어서 깊이 있는 해석과 위로를 해주고, 상황에 맞는 구절을 인용해줘. 대화 톤은 매우 차분하고 신중하며, 따뜻한 포용력이 느껴져야 해. 불필요한 웃음소리는 제외하고 진중하게 답변해줘.";
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: '연결이 고르지 못합니다. 잠시 후 다시 시도해 주세요.' });
    }
});

app.listen(port, () => console.log(`My Pray server running at http://localhost:${port}`));
